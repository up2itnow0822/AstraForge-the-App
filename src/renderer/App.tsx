...
import ComposerPanel from './components/composer/ComposerPanel'; // Import Composer
...
const App: React.FC = () => {
    ...
    const handleDebate = async () => {
        ...
        try {
            ...
            // Auto switch to composer on debate start if appropriate, or keep manual
        } catch (e) {
            ...
            <div className="flex-1 flex flex-col">
                <div className="flex-1 p-6 flex flex-col">
                    <div className="flex justify-between items-center mb-4">
                        <h1 className="text-2xl font-bold text-white">AstraForge IDE</h1>
                        {/* View Toggle */}
                        <div className="flex bg-gray-800 rounded p-1 gap-1">
                            <button 
                                onClick={() => setViewMode('editor')}
                                className={`px-3 py-1 rounded text-xs font-bold ${viewMode === 'editor' ? 'bg-astra-blue text-white' : 'text-gray-400 hover:text-white'}`}
                            >
                                EDITOR
                            </button>
                            <button 
                                onClick={() => setViewMode('composer')}
                                className={`px-3 py-1 rounded text-xs font-bold ${viewMode === 'composer' ? 'bg-astra-blue text-white' : 'text-gray-400 hover:text-white'}`}
                            >
                                COMPOSER
                            </button>
                        </div>
                    ...
                    <div className="flex-1 bg-gray-900 border border-gray-700 rounded overflow-hidden">
                        {viewMode === 'editor' ? <EditorComponent /> : <ComposerPanel />}
                    </div>
                </div>
                <Terminal logs={logs} />
            </div>
