...
const App: React.FC = () => {
  ...
  useEffect(() => {
    window.astraAPI.onAgentUpdate((data: any) => {
      if (data.type === 'log') {
        setLogs(prev => [...prev, data.message]);
      }
      if (data.type === 'status') {
        // Update agent status (e.g., thinking/speaking)
        setAgents(prev => prev.map(agent => 
          agent.id === data.agentId ? { ...agent, status: data.status } : agent
        ));
      }
      // Reset all to idle if phase changes or ends, or specialized events occur
       if (data.type === 'phase' && data.phase === 'voting_concluded') {
           setAgents(prev => prev.map(a => ({ ...a, status: 'idle' })));
       }
    });
    return () => window.astraAPI.removeAgentUpdateListener();
  }, []);
