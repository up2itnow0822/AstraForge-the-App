...
class SecurityScanner {
  constructor(rootDir = process.cwd()) {
    ...
    // Suspicious logging patterns (excluding false positives)
    this.suspiciousLogPatterns = [
      {
        name: 'API Key Logging',
        pattern: /console\.(log|info|debug|warn|error)\s*\([^)]*(?:apiKey|API_KEY|api.*key)[^)]*\)/gi,
        severity: 'HIGH'
      },
      {
        name: 'Authorization Header Logging',
        pattern: /console\.(log|info|debug|warn|error)\s*\([^)]*(?:Authorization|Bearer)[^)]*process\.env|headers/gi,
        severity: 'HIGH'
      },
      {
  ...
  async scanForSuspiciousLogging() {
    const issues = [];
    ...
    for (const file of files) {
      ...
      for (const pattern of this.suspiciousLogPatterns) {
        ...
        for (const match of matches) {
          ...
          // Skip security documentation and scanner files
          if (file.includes('security-scan') || 
              (file.includes('SECURITY') && /\[REDACTED\]|documentation|example/i.test(line))) {
            continue;
          }
  ...
  isFakeTestKey(key, file, line) {
    ...
    if (!isTestFile) {
      // Also check if it's in documentation about security issues
      if (file.includes('SECURITY') || file.includes('security')) {
        return /\[REDACTED\]|example|documentation/i.test(line);
      }
      return false;
    }
