...
const LanceDBClient_1 = require("../../src/core/storage/LanceDBClient");
...
describe('LanceDBClient', () => {
    ...
    beforeEach(async () => {
        ...
        client = new LanceDBClient_1.LanceDBClient(testDbPath);
    ...
    describe('Search Operations', () => {
        ...
        it('should search with metadata filter', async () => {
            const results = await client.searchWithFilter('learning', { category: 'ML' }, 5);
            expect(results.length).toBeGreaterThan(0);
            expect(results.every(doc => doc.metadata.category === 'ML')).toBe(true);
        });
        it('should search with complex metadata filter', async () => {
            const results = await client.searchWithFilter('learning', { category: 'programming', difficulty: 'intermediate' }, 5);
            expect(results.length).toBeGreaterThan(0);
            expect(results.every(doc => doc.metadata.category === 'programming' &&
                doc.metadata.difficulty === 'intermediate')).toBe(true);
        });
    ...
    describe('Error Handling', () => {
        it('should handle search before initialization gracefully', async () => {
            const newClient = new LanceDBClient_1.LanceDBClient(testDbPath);
            // Don't initialize
            await expect(newClient.search('test')).rejects.toThrow();
        });
        it('should handle update of non-existent document', async () => {
            await expect(client.updateDocument('non-existent', { content: 'updated' })).rejects.toThrow('Document not found');
        });
        it('should handle invalid input validation', async () => {
            await expect(
            // @ts-ignore - Testing invalid input
            client.addDocument(123)).rejects.toThrow();
        });
    });
    describe('Pagination', () => {
        beforeEach(async () => {
